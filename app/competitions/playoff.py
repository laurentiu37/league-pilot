...
class PlayoffEngine:
    ...
    @staticmethod
    def _next_round_start(games: List[Game], fallback: date, rest_days: int = 2) -> date:
        played_dates = [g.slot.date for g in games if g.slot is not None]
        if not played_dates:
            return fallback + timedelta(days=rest_days)
        return max(played_dates) + timedelta(days=rest_days)
    ...
    def run_playoff_top8(self, top8: List[Team]) -> Tuple[List[Game], List[Team]]:
        ...
        sf_start = self._next_round_start(all_games, start)

        # SF Bo5 (fixed bracket)
        ...
        medal_start = self._next_round_start(all_games, sf_start)
        ...
        placement_final_start = self._next_round_start(all_games, placement_5_8_start)
    ...
    def run_playout_bottom8(self, bottom8: List[Team]) -> Tuple[List[Game], List[Team]]:
        ...
        placement_9_12_start = self._next_round_start(all_games, sf_start)
        ...
        placement_13_16_final_start = self._next_round_start(all_games, placement_13_16_start)
